...
public class DeepSeekClient {
    ...
    public void init() {
        ...
        if (deepseekBaseUrl != null && !deepseekBaseUrl.isEmpty()
            ...
            log.warn("DeepSeek 未配置，文书生成功能不可用");
        }
    }

    /**
     * 调用 DeepSeek Chat API 生成文书内容
     *
     * @param systemPrompt 系统提示词（角色设定、任务说明）
     * @param userPrompt   用户提示词（案件事实、模板结构等）
     * @return AI 生成的文书内容
     */
            ...
            try (Response response = okHttpClient.newCall(request).execute()) {
                ...
                if (choices != null && !choices.isEmpty()) {
                    String content = choices.getJSONObject(0)
                            ...
                            .getString("content");
                    log.info("文书生成成功，内容长度: {} 字符", content != null ? content.length() : 0);
                    return content;
                }
